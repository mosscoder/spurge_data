...
def save_study_cb(study, trial):
    ...
    if trial.state == optuna.trial.TrialState.COMPLETE:
        opt_hist_path = os.path.join(results_dir, f'aug_opt_history_plot.png')
        fig1 = optuna.visualization.plot_optimization_history(study)
        fig1.write_image(opt_hist_path)

        opt_slice_path = os.path.join(results_dir, f'aug_opt_slice_plot.png')
        fig2 = optuna.visualization.plot_slice(study)
        fig2.write_image(opt_slice_path)
        
        if len(study.trials) > 1:
            opt_importance_path = os.path.join(results_dir, f'aug_opt_importance_plot.png')
            fig3 = optuna.visualization.plot_param_importances(study)
            fig3.write_image(opt_importance_path)

    study_path = os.path.join(results_dir, 'aug_opt_study.pkl')
...
best_params_path = './results/best_augs.json'
